...
var wordSubsets = function (A, B) {
    ...
    B.forEach(b => {
        ...
        [...b].forEach(ch => tmp[ch.charCodeAt(0) - aCharCode]++);
        arr.forEach((_, i) => arr[i] = Math.max(arr[i], tmp[i]));
    });

    let isSubset = (w) => {
        let tmp = Array(26).fill(0);
        [...w].forEach(ch => tmp[ch.charCodeAt(0) - aCharCode]++);
        return tmp.every((_, i) => tmp[i] >= arr[i]);
    }

    return A.filter(word => isSubset(word));
};
